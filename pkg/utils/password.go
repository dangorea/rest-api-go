...
func VerifyPassword(password, encodedHash string) error {
	...
	if len(parts) != 2 {
		return ErrorHandler(errors.New("invalid encoded password hash format"), "Internal server error")
	}
...
func HashPassword(password string) (string, error) {
	if password == "" {
		return "", ErrorHandler(errors.New("please is blank"), "please enter password")
	}
	salt := make([]byte, 16)
